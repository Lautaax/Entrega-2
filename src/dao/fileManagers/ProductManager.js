...
export default class ProductManager {
  ...
  addProduct = async (productObject) => {
    try {
      ...
      const productIndex = await products.findIndex(
        (prod) => prod.code === productObject.code
      );
  ...
  getProductById = async (id) => {
    try {
      if (fs.existsSync(this.path)) {
        ...
        let indexValue = result.find(
          (event) => event.id === Number.parseInt(id)
        );
  ...
  deleteProducts = async (id) => {
    try {
      ...
      const products = await this.getProducts();
      let productFounded = products.findIndex(
  ...
  updateProduct = async (idUpdate, productUpdate) => {
    try {
      ...
      let productExists = products.findIndex(
        (product) => product.id === Number.parseInt(idUpdate)
      );
      if (productExists !== -1) {
        const products = await this.getProducts();
        const productIdFound = products.findIndex(
          (prod) => prod.id === parseInt(idUpdate)
        );

        if (productIdFound !== -1) {
          const updatedProduct = {
            ...products[productIdFound],
            ...productUpdate,
          };
